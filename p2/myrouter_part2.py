    ...
    def check_queue(self):
        for ip in self.queue:#traverse ordered dict
            ...
            if time.time() - self.queue[ip][3] <= 1:
                ...
                destIP = ip
                self.net.send_packet(intf,create_ip_arp_request(senderhwaddr/srcMAC,senderprotoaddr/srcIP,targetprotoaddr/destIP))
            ...
            if gotpkt == False:
                ...
                if arp:
                    ...
                    #handle arp reply
                        ...
                        if arp.senderprotoaddr in self.queue:#if the ip is already recorded
                            ...
                            for packet in self.queue[arp.senderprotoaddr][0]:#complete eth header for each packet buffered by this ip and send out
                                if not packet[0]:
                                    packet+=Ethernet()
                ...
                if ipv4_h:
                    ...
                    if ipv4_h.dst in my_ip:
                        continue
                    #In case of two items in the table matching, the longest prefix match should be used.
                    max = -1
                    match = None
                    ...
                    intf_name = match[3]
                    if next_hop in self.arp_table:#MAC address stored in switch cache, use it.
                        if not pkt[0]:
                            pkt+=Ethernet()
                        ...
                        self.arp_table[next_hop][1] = time.time() 
                    else:#not in cache
